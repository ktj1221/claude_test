        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-6">
            ...
            <nav className="flex items-center gap-1">
              <Link href="/admin/rental" className="px-3 py-1.5 text-slate-500 hover:bg-slate-100 text-sm rounded-lg transition-colors">
                대여 관리
              </Link>
              <span className="px-3 py-1.5 bg-indigo-50 text-indigo-700 text-sm font-medium rounded-lg">장비 관리</span>
            </nav>
          </div>
      ...
      <main className="max-w-7xl mx-auto px-4 py-6">
        {loading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {[1, 2, 3].map(i => <div key={i} className="bg-white rounded-xl h-48 border border-slate-200 animate-pulse" />)}
          </div>
        ) : equipment.length === 0 ? (
          <div className="text-center py-20">
            <p className="text-slate-400 mb-4">등록된 장비가 없습니다.</p>
            <button onClick={openAdd} className="px-5 py-2.5 bg-indigo-600 text-white rounded-lg text-sm font-medium hover:bg-indigo-700">
              첫 장비 추가
            </button>
          </div>
        ) : (
              ...
              <div key={eq.id} className="bg-white rounded-xl border border-slate-200 overflow-hidden hover:shadow-md transition-shadow">
                ...
                <div className="p-4">
                  <div className="flex items-start justify-between mb-1">
                    ...
                    <span className={`text-xs px-2 py-0.5 rounded-full shrink-0 ml-2 ${eq.is_available ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                  ...
                  <div className="flex gap-2 mt-3 pt-3 border-t border-slate-100">
                    ...
                    <button
                      ...
                      className="flex-1 py-1.5 text-xs font-medium text-red-600 border border-red-200 rounded-lg hover:bg-red-50"
                    >
                      삭제
                    </button>
                  </div>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSave} className="p-5 space-y-4">
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">장비명 <span className="text-red-500">*</span></label>
                <input
                  type="text"
                  value={form.name}
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">시리얼 번호</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">장비 사진</label>
                <div
                ...
                >
                  {photo ? (
                    ...
                    <div className="text-slate-400">
                      ...
                      <p className="text-sm">사진 업로드 (최대 5MB)</p>
                    </div>
                  )}
                </div>
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-xl p-6 max-w-sm w-full">
            ...
            <p className="text-sm text-slate-500 mb-5">이 장비를 삭제하시겠습니까? 진행 중인 대여가 있으면 삭제할 수 없습니다.</p>
